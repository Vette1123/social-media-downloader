...
| 2026-08-20 | [The close button we broke ourselves](2026-08-20-bmc-widget-close-zindex.md) | The tip jar's chevron is a picture, not a button — the vendor closes via an overlay stacked above it, so our `body > div:not([class])` z-index band flattened both to one layer and DOM order handed the tap back to "open". |
