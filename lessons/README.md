...
| 2026-08-15 | [Instagram closed the logged-out door](2026-08-15-instagram-logged-out-wall.md) | Every login-free Instagram surface is now shut (embed shell, retired `doc_id` — even a freshly harvested one, `__a=1` 404, Cobalt 400), so the only working path is the one reserved for the `ig` grant — and the log blamed a missing secret that was set. |
