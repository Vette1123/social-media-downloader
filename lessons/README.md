...
| 2026-08-19 | [The tip jar the framework threw away](2026-08-19-bmc-widget-static-export.md) | React 19 silently drops a non-async `<script src>` from the prerender, and the vendor bundle only builds inside a `DOMContentLoaded` it will never hear — so the tag has to be injected after `load` and handed a synthetic event. |
